...
with open('scraper_code/input.txt', 'r', encoding='utf-8') as input_file:
...
for match in re.finditer(pattern, input_text, re.DOTALL):
    ...
    for word in words_to_check:
        if '\nภาคที่ ' in detail or '\nภาค ' in detail:
            ...
            pending_replacements['part'] = add_brackets_around_number(detail.split('\nส่วนที่', 1) if '\nส่วนที่' in detail else detail.split('\nส่วน', 1) [1].strip())
            detail = parts[0]
            pending_replacements['additional'] = '-'
...
with open('scraper_code/output-new.csv', 'w', newline='', encoding='utf-8-sig') as csv_file:
